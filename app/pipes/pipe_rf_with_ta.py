...
datas = pd.read_csv("app/data/DatasetRF.csv")
...
def trix(df, n):
    EX1 = close.ewm(span=n, min_periods=n).mean()
    EX2 = EX1.ewm(span=n, min_periods=n).mean()
    EX3 = EX2.ewm(span=n, min_periods=n).mean()
    df1 = df.reset_index(drop=True)
    i = 0
    ROC_l = [np.nan]
    while i + 1 <= df1.index[-1]:
...
def average_directional_movement_index(df, n, n_ADX):
    ...
    while i + 1 <= df.index[-1]:
        UpMove = df.loc[i + 1, 'High_BTC'] - df.loc[i, 'High_BTC']
        DoMove = df.loc[i, 'Low_BTC'] - df.loc[i + 1, 'Low_BTC']
    ...
    TR_l = [0]
    while i < df.index[-1]:
        TR = max(df.loc[i + 1, 'High_BTC'], df.loc[i, 'Price_BTC']) - min(df.loc[i + 1, 'Low_BTC'], df.loc[i, 'Price_BTC'])
        TR_l.append(TR)
        i = i + 1
    TR_s = pd.Series(TR_l)
...
def vortex_indicator(df, n):
    ...
    df = df.reset_index(drop=True)
    while i < df.index[-1]:
        Range = max(df.loc[i + 1, 'High_BTC'], df.loc[i, 'Price_BTC']) - min(df.loc[i + 1, 'Low_BTC'], df.loc[i, 'Price_BTC'])
        TR.append(Range)
        i = i + 1
    i = 0
    VM = [0]
    while i < df.index[-1]:
        Range = abs(df.loc[i + 1, 'High_BTC'] - df.loc[i, 'Low_BTC']) - abs(df.loc[i + 1, 'Low_BTC'] - df.loc[i, 'High_BTC'])
        VM.append(Range)
        i = i + 1
    VI = pd.Series(pd.Series(VM).rolling(n).sum() / pd.Series(TR).rolling(n).sum(), name='Vortex')
    df = df.join(VI)
    return df
...
def on_balance_volume(df, n):
    ...
    df = df.reset_index(drop=True)
    while i < df.index[-1]:
        if df.loc[i + 1, 'Price_BTC'] - df.loc[i, 'Price_BTC'] > 0:
            OBV.append(df.loc[i + 1, 'Volume_BTC'])
        if df.loc[i + 1, 'Price_BTC'] - df.loc[i, 'Price_BTC'] == 0:
            OBV.append(0)
        if df.loc[i + 1, 'Price_BTC'] - df.loc[i, 'Price_BTC'] < 0:
            OBV.append(-df.loc[i + 1, 'Volume_BTC'])
...
df = trix(df, n)
df = average_directional_movement_index(df, n, n)
df = vortex_indicator(df, n)
df = relative_strength_index(df, n)
df = on_balance_volume(df, n)
...
df = df.drop(columns=['Force_Index', 'Chaikin', 'MACDdiff',
                      ...
                      'Price_ETH', 'High_DASH', 'Open_DASH', 'Low_DASH', 'Trix', 'ADX', 'Vortex',
                      'RSI', 'OBV', 'GT_buy_bitcoin', 'GT_bitcoin', 'ADL'])
"""""
df = df.dropna()
df = df.reset_index(drop=True)
print(df.shape)
