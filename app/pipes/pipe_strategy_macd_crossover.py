...
from app.strategies.strategy_macd_crossover import StrategyMacdCrossover
...
class PipeSimpleMA:
    ...
    def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['ema_200'] = TA.EMA(df, period=200)
        df['comparison'] = np.where(df['macd'] > df['signal'], 1, 0)
        df['cross'] = df['comparison'].diff()
        return df
    ...
    def plot_frame(self, df: pd.DataFrame) -> pd.DataFrame:
        Plotter(df, plot_name="strategy_macd_crossover")
        return df
